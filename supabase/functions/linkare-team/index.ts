...
Deno.serve(async request=>{
 ...
 try{
  ...
  const raw=await request.text();if(raw.length>16384)throw new ApiError(413,'Solicitud demasiado grande.');const input=JSON.parse(raw);const {user,db}=await requireMember(request,input.organizationId,'owner');
  ...
  if(action==='list'){
   ...
   const users=await Promise.all((members||[]).map(async m=>{const {data}=await db.auth.admin.getUserById(m.user_id);return {id:m.user_id,name:m.display_name,email:data.user?.email||'',role:uiRole(m.role),active:m.active,title:m.professional_title,phone:m.phone,permissions:m.permissions};}));
   const {data:invitations,error:e}=await db.from('linkare_invites_v3').select('id,email,display_name,requested_role,professional_title,phone,permissions,status,delivery_status,expires_at,created_at').eq('organization_id',org).eq('status','pending');if(e)throw e;
  ...
  if(action==='invite'||action==='resend'){
   ...
   if(action==='resend' && (!existing || existing.id!==input.inviteId))throw new ApiError(404,'La invitación ya no está pendiente.');
   if(action==='invite' && existing)throw new ApiError(409,'Ya hay una invitación pendiente. Use Reenviar o Revocar.');
   const role=action==='resend'?existing.requested_role:requestedRole(input.role);
   let invite=existing;
   if(!invite){const {data,error}=await db.from('linkare_invites_v3').insert({organization_id:org,email,display_name:name,requested_role:role,professional_title:clean(input.title),phone:clean(input.phone,40),permissions:permissions(input.permissions,role),invited_by:user.id}).select().single();if(error)throw new ApiError(409,'No se pudo reservar la invitación. Actualice el equipo y vuelva a intentarlo.');invite=data;}
   ...
   const delivery=await db.from('linkare_invites_v3').update({delivery_status:deliveryError?'failed':'sent'}).eq('id',invite.id).eq('organization_id',org);if(delivery.error)throw delivery.error;
  ...
  if(action==='update'||action==='toggle'){
   ...
   if(!member||!['psychiatrist','clinical_assistant','secretary'].includes(member.role))throw new ApiError(403,'No puede modificar al propietario ni usuarios ajenos.');
   const role=action==='update'?requestedRole(input.role):member.role;
   const changes: Record<string,unknown>=action==='toggle'?{active:!member.active}:{role,display_name:clean(input.name),professional_title:clean(input.title),phone:clean(input.phone,40),permissions:permissions(input.permissions,role)};
   if(action==='update'&&!changes.display_name)throw new ApiError(400,'Indique un nombre.');
   const {error}=await db.from('organization_members').update(changes).eq('id',member.id).eq('organization_id',org);if(error)throw error;
