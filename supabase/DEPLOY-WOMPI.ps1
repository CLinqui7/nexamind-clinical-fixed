...
if (-not $ProjectRef) {
  $ProjectRef = Read-Host "Supabase project ref (ej. abcdefghijklmnop)"
}
...
$requireAuth = Read-Host "¿Exigir Supabase Auth? Escriba true o false (producción: true)"
if (-not $requireAuth) { $requireAuth = "true" }
...
WOMPI_REQUIRE_AUTH=$requireAuth
...
try {
  ...
  Write-Host "Wompi Edge Functions desplegadas correctamente." -ForegroundColor Green
  Write-Host "Webhook para Wompi:" -ForegroundColor Cyan
