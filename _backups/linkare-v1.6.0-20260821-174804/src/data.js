...
export function createSeedData() {
  ...
  return normalizeData({
    ...
    users: [
      { id: 'user_owner_1', name: 'Administración Linkare', email: 'admin@linkare.app', password: 'LinkareAdmin2026!', phone: '', title: 'Propietaria de la plataforma', role: 'owner', active: true, avatar: '', permissions: {}, createdAt: new Date().toISOString() },
...
export function normalizeData(input = {}) {
  ...
  const users = Array.isArray(input.users) && input.users.length
    ? input.users.map((user, index) => ({
        ...
        email: user.email || '', password: user.password || (user.role === 'owner' ? 'LinkareAdmin2026!' : user.role === 'doctor' ? 'NexaMind2026!' : 'Agenda2026!'), phone: user.phone || '', title: user.title || '',
  ...
  return {
    ...
    billing: {
      ...
      wompiPublicKey: input.billing?.wompiPublicKey || '',
      wompiCheckoutUrl: input.billing?.wompiCheckoutUrl || '',
      wompiRedirectUrl: input.billing?.wompiRedirectUrl || '',
