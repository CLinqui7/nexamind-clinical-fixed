...
const TUTORIAL_STORAGE_KEY = 'linkare-tutorial-v2';
...
class App extends React.Component {
  ...
  fillDemoCredentials = role => {
    ...
    this.setState({
      loginDraft: {
        email: user.email || '',
        password: user.password || (role === 'doctor' ? 'NexaMind2026!' : 'Agenda2026!'),
        showPassword: false,
      },
      loginError: '',
    });
  ...
  renderTopbar() {
    ...
    const nav = [
      ...
      ['payments', 'insurance', 'Cobros', this.can('appointmentsManage') || this.can('settingsManage')],
  ...
  renderPayments() {
    const canPayments = this.can('appointmentsManage') || this.can('settingsManage');
    if (!canPayments) return html`<${EmptyState} icon="insurance" title="Acceso restringido" text="Este usuario no tiene permiso para consultar cobros."/>`;
    const billing = this.state.data.billing || {};
    const payments = Array.isArray(this.state.data.payments) ? this.state.data.payments : [];
    const patients = Array.isArray(this.state.data.patients) ? this.state.data.patients : [];
    const appointments = Array.isArray(this.state.data.appointments) ? this.state.data.appointments : [];
    ...
    const methods = [billing.wompiEnabled && 'Wompi', billing.transferEnabled && 'Transferencia', billing.cashEnabled && 'Efectivo', billing.insuranceEnabled && 'Seguro'].filter(Boolean);
    return html`<div className="view-enter"><${PageHeader} eyebrow="Cobros y métodos de pago" title="Wompi, tarifas y pagos" subtitle="Configure cómo cobrará las consultas y deje listo el enlace de pago." actions=${html`<div className="tour-actions-group">${this.can('settingsManage') ? html`<${Button} tone="secondary" icon="settings" onClick=${this.openBillingSettings}>Configurar pagos</${Button}>` : null}${billing.wompiCheckoutUrl ? html`<${Button} icon="external" onClick=${() => this.openPaymentLink(billing.wompiCheckoutUrl)}>Abrir Wompi</${Button}>` : null}</div>`}/>
      <div className="kpi-grid"><${KpiCard} label="Pagos pendientes" value=${pending.length} hint="requieren seguimiento" icon="clock" tone=${pending.length ? 'coral' : 'teal'}/><${KpiCard} label="Pagos recibidos" value=${paid.length} hint="registrados" icon="check" tone="teal"/><${KpiCard} label="Tarifa consulta" value=${money(billing.consultationFee)} hint="consulta inicial" icon="file" tone="blue"/><${KpiCard} label="Métodos activos" value=${methods.length} hint=${methods.join(' · ') || 'Sin configurar'} icon="insurance" tone="purple"/></div>
      <div className="dashboard-grid"><${Card} className="span-7" title="Configuración actual" subtitle="Puede empezar con un checkout URL y después conectar backend y webhook."><div className="billing-summary-grid"><div className="billing-summary-card"><span>Consulta inicial</span><strong>${money(billing.consultationFee)}</strong><small>${currency}</small></div><div className="billing-summary-card"><span>Seguimiento</span><strong>${money(billing.followupFee)}</strong><small>Tarifa editable</small></div><div className="billing-summary-card"><span>Wompi</span><strong>${billing.wompiEnabled ? 'Activo' : 'Pendiente'}</strong><small>${billing.wompiCheckoutUrl ? 'Enlace configurado' : 'Agregue checkout URL'}</small></div></div><div className="method-badges">${methods.map(method => html`<${Badge} key=${method} tone="blue">${method}</${Badge}>`)}</div><div className="clinical-footnote"><${Icon} name="shield" size=${17}/> Para cobro real y seguro: backend crea la orden, Wompi confirma por webhook y Supabase actualiza el pago.</div></${Card}><${Card} className="span-5" title="Atajos de cobro"><div className="settings-actions vertical-actions">${billing.wompiCheckoutUrl ? html`<${Button} icon="external" onClick=${() => this.openPaymentLink(billing.wompiCheckoutUrl)}>Abrir Wompi</${Button}><${Button} tone="secondary" onClick=${() => this.copyPaymentLink(billing.wompiCheckoutUrl)}>Copiar enlace</${Button}>` : html`<${Button} tone="secondary" icon="settings" onClick=${this.openBillingSettings}>Agregar enlace</${Button}>`}<${Button} tone="soft" onClick=${() => this.setView('agenda')}>Ir a agenda</${Button}></div><div className="notes-box"><span>Transferencia</span><p>${billing.transferInstructions || 'Agregue instrucciones bancarias.'}</p></div></${Card}><${Card} className="span-12" title="Registro de pagos"><div className="table-wrap"><table className="data-table"><thead><tr><th>Paciente</th><th>Descripción</th><th>Método</th><th>Monto</th><th>Estado</th><th>Fecha</th></tr></thead><tbody>${payments.length ? payments.map(item => { const patient = patients.find(p => p.id === item.patientId); const appointment = appointments.find(a => a.id === item.appointmentId); return html`<tr key=${item.id}><td><b>${patient?.name || 'Paciente'}</b><small>${appointment?.type || 'Consulta'}</small></td><td>${item.description}</td><td>${item.method}</td><td>${money(item.amount)}</td><td><${Badge} tone=${item.status === 'paid' ? 'success' : 'warning'}>${item.status === 'paid' ? 'Pagado' : 'Pendiente'}</${Badge}></td><td>${formatDateTime(item.createdAt)}</td></tr>`; }) : html`<tr><td colSpan="6">Sin pagos.</td></tr>`}</tbody></table></div></${Card}></div></div>`;
  }
  ...
  renderBillingSettingsModal() {
    const draft = this.state.modal?.draft || {};
    return html`<${Modal} title="Métodos de pago" subtitle="Configure tarifas y Wompi. La conexión 100% real requiere backend y webhook." onClose=${this.closeModal} size="lg"><form className="clinical-form" onSubmit=${this.saveBillingSettingsForm}>${this.renderModalError()}<fieldset><legend>Tarifas</legend><div className="form-grid"><${FormField} label="Consulta inicial"><input type="number" step="0.01" value=${draft.consultationFee} onChange=${event => this.updateDraft('consultationFee', event.target.value)}/></${FormField}><${FormField} label="Seguimiento"><input type="number" step="0.01" value=${draft.followupFee} onChange=${event => this.updateDraft('followupFee', event.target.value)}/></${FormField}></div><${FormField} label="Moneda"><input value=${draft.currency} onChange=${event => this.updateDraft('currency', event.target.value)}/></${FormField}></fieldset><fieldset><legend>Wompi</legend><label className="form-checkbox-card"><input type="checkbox" checked=${Boolean(draft.wompiEnabled)} onChange=${event => this.updateDraft('wompiEnabled', event.target.checked)}/><span><b>Activar Wompi</b><small>Mostrará el enlace en Cobros.</small></span></label><div className="form-grid"><${FormField} label="Public key"><input value=${draft.wompiPublicKey} onChange=${event => this.updateDraft('wompiPublicKey', event.target.value)}/></${FormField}><${FormField} label="Checkout URL"><input value=${draft.wompiCheckoutUrl} onChange=${event => this.updateDraft('wompiCheckoutUrl', event.target.value)}/></${FormField}></div><${FormField} label="URL de retorno"><input value=${draft.wompiRedirectUrl} onChange=${event => this.updateDraft('wompiRedirectUrl', event.target.value)}/></${FormField}></fieldset><fieldset><legend>Otros métodos</legend><div className="form-grid"><label className="form-checkbox-card"><input type="checkbox" checked=${Boolean(draft.transferEnabled)} onChange=${event => this.updateDraft('transferEnabled', event.target.checked)}/><span><b>Transferencia</b></span></label><label className="form-checkbox-card"><input type="checkbox" checked=${Boolean(draft.cashEnabled)} onChange=${event => this.updateDraft('cashEnabled', event.target.checked)}/><span><b>Efectivo</b></span></label></div><label className="form-checkbox-card"><input type="checkbox" checked=${Boolean(draft.insuranceEnabled)} onChange=${event => this.updateDraft('insuranceEnabled', event.target.checked)}/><span><b>Seguro</b></span></label><${FormField} label="Instrucciones de transferencia"><textarea rows="3" value=${draft.transferInstructions} onChange=${event => this.updateDraft('transferInstructions', event.target.value)}></textarea></${FormField}></fieldset><${FormField} label="Nota interna"><textarea rows="2" value=${draft.note} onChange=${event => this.updateDraft('note', event.target.value)}></textarea></${FormField}><div className="clinical-footnote"><${Icon} name="shield" size=${17}/> No coloque llaves secretas en el frontend. Use únicamente public key y guarde secretos en backend.</div><${FormActions} onCancel=${this.closeModal} submitLabel="Guardar pagos"/></form></${Modal}>`;
  }
  ...
  renderGuidedTour() {
    ...
    return html`<div className="guided-tour compact-guided-tour" role="dialog" aria-modal="true"><div className="tour-dim compact-dim"></div><aside className="tour-card compact-card"><header className="tour-card-head"><div className="tour-guide"><div className="guide-orb tiny"><span></span><span></span><span></span></div><div><span>${step.chapter}</span><b>Paso ${this.state.tourIndex + 1} de ${steps.length}</b></div></div><button className="tutorial-close" onClick=${this.exitTour}><${Icon} name="close"/></button></header><div className="tour-progress"><i style=${{ width: `${progress}%` }}></i></div><div className="tour-card-body"><span className="tour-step-icon"><${Icon} name=${step.icon || 'help'} size=${20}/></span><h2>${step.title}</h2><p>${step.description}</p><div className="tour-context-chip">Pantalla: <b>${step.view === 'payments' ? 'Cobros' : step.view === 'patient' ? 'Expediente' : step.view === 'analytics' ? 'Resultados' : step.view === 'dashboard' ? 'Inicio' : step.view}</b></div>${step.items?.length ? html`<ul>${step.items.map(item => html`<li key=${item}><${Icon} name="check" size=${15}/><span>${item}</span></li>`)}</ul>` : null}</div><footer className="tour-card-footer"><button className="tour-exit" onClick=${this.exitTour}>Salir</button><div><${Button} tone="secondary" disabled=${this.state.tourIndex === 0} onClick=${this.previousTourStep}>Anterior</${Button}><${Button} icon=${isLast ? 'check' : 'chevronRight'} onClick=${this.nextTourStep}>${isLast ? 'Finalizar' : 'Siguiente'}</${Button}></div></footer></aside></div>`;
  }
            ...
            : view === 'payments' ? this.renderPayments()
              : view === 'analytics' ? this.renderAnalytics()
                : view === 'alerts' ? this.renderAlerts()
                  : this.renderSettings();
    return html`<div className=${`app-shell ${settings.largeText ? 'large-text-mode' : ''} ${settings.reducedMotion ? 'reduced-motion-mode' : ''}`}><div className="ambient ambient-one"></div><div className="ambient ambient-two"></div><div className="app-frame">${this.renderTopbar()}<main className="main-content">${body}</main><footer><span>Linkare · Apoyo al seguimiento</span><span>Datos sintéticos · No usar para atención real</span></footer></div>${this.renderModal()}${this.renderAppointmentDetails()}${this.renderTutorialIntro()}${this.renderGuidedTour()}${this.state.toast ? html`<div className=${`toast toast-${this.state.toastTone}`}><${Icon} name=${this.state.toastTone === 'danger' ? 'alert' : 'check'} size=${18}/>${this.state.toast}</div>` : null}</div>`;
  }
}
