...
Permissions are enforced by SQL/Edge Functions, not hidden buttons. Secretary access must never include private clinical records. Use versioned per-resource deltas; never overwrite the clinic snapshot blindly. Signed notes must remain byte-preserving objects across normalization. Documents are archived, not destroyed by browser actions.
