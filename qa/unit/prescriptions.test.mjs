...
import {archivePrescription,savePrescription} from '../../src/practice.js';
...
for(const role of ['doctor','secretary'])test(role+' can archive an existing prescription without removing its record',()=>{
 const d=data(role);const r=archivePrescription(d,'p',old.id);assert.equal(r.data.patients[0].prescriptions.length,1);assert.ok(r.prescription.archivedAt);assert.equal(r.prescription.archivedBy,role);assert.equal(d.patients[0].prescriptions[0].archivedAt,undefined);assert.throws(()=>savePrescription(r.data,'p',{...r.prescription,date:'2026-09-21'}),/borrada/);
});
