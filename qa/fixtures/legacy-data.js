import { defaultAnnualDates, normalizePatientV2 } from '../../src/v2features.js';
...
export function normalizeData(input = {}) {
  const defaultDoctor = {
    ...
    email: 'doctora@nexamind.demo', password: '', phone: '', title: input.organization?.specialty || 'Psiquiatría',
  ...
  const users = Array.isArray(input.users) && input.users.length
    ? input.users.map((user, index) => ({
        ...
        email: user.email || '', password: user.password || (user.role === 'owner' ? '' : user.role === 'doctor' ? '' : ''), phone: user.phone || '', title: user.title || '',
