...
El médico con `medicationsManage` revisa la captura y puede activarla. El servidor fija `reviewedBy`, `reviewedAt` y el evento `approved`. Cambiar dosis, frecuencia o estado conserva el registro previo en `events` y `doseHistory`.
...
Las capturas generan `medication.captured` en `linkare_audit_v3`; los demás guardados conservan el fingerprint y la revisión optimista del expediente. La migración es aditiva. Para una corrección se debe crear otra migración; no se debe eliminar el historial.
