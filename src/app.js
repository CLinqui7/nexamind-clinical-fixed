...
function FormActions({ onCancel, submitLabel = 'Guardar', dangerAction = null, disabled = false }) {
  return html`<div className="form-actions">${dangerAction ? html`<div>${dangerAction}</div>` : html`<div></div>`}<div><${Button} key="cancel" tone="secondary" disabled=${disabled} onClick=${onCancel}>Cancelar</${Button}><${Button} key="submit" icon="check" type="submit" disabled=${disabled}>${disabled?'Guardando…':submitLabel}</${Button}></div></div>`;
}
