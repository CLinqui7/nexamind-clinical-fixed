...
class App extends React.Component {
  ...
  deleteStoredDocument = async (patientId, document) => {
    if (!this.can('documentsManage')) return this.permissionDenied();
    try {
      await deletePatientDocumentFile(document);
      const data = {
        ...this.state.data,
      ...
      this.persistDataUpdate({ data }, 'Documento archivado. El archivo original se conserva.');
    } catch (error) {
  ...
  saveAssessmentForm = event => {
    ...
    try {
      ...
      this.persistDataUpdate({ data: result.data, patientTab: 'followup', modal: null, modalError: '' }, 'Evolución clínica registrada.');
    } catch (error) { this.handleFormError(error); }
  };
  ...
  saveVitalsForm = event => {
    ...
    try {
      ...
      this.persistDataUpdate({ data: result.data, patientTab: 'followup', modal: null, modalError: '' }, 'Control físico registrado.');
    } catch (error) { this.handleFormError(error); }
  };
  ...
  saveAdverseForm = event => {
    ...
    try {
      ...
      this.persistDataUpdate({ data: result.data, patientTab: 'safety', modal: null, modalError: '' }, 'Efecto observado registrado.');
    } catch (error) { this.handleFormError(error); }
  };
  ...
  saveLabForm = event => {
    ...
    try {
      ...
      this.persistDataUpdate({ data: result.data, patientTab: 'safety', modal: null, modalError: '' }, 'Resultado de laboratorio agregado.');
    } catch (error) { this.handleFormError(error); }
  };
  ...
  openMedicationDelete = (patient,medication) => {
    if(!this.can('medicationsManage'))return this.permissionDenied();
    this.setState({modal:{type:'medicationDelete',patientId:patient.id,medicationId:medication.id,draft:{reason:''}},modalError:''});
  };

  saveMedicationDelete = async event => {
    event.preventDefault();const {patientId,medicationId,draft}=this.state.modal;this.setState({formSaving:true,modalError:''});
    try{await archiveMedication(this.state.remoteOrganizationId,patientId,medicationId,draft.reason);const session=await getProductionSession();await this.applyProductionSession(session);this.setState({view:'patient',selectedPatientId:patientId,patientTab:'medications',formSaving:false,modal:null});this.notify('Medicamento eliminado de la lista y conservado en el historial.');}
    ...
    return html`<div className="view-enter">
      ...
      ${this.state.patientTab === 'medications' ? html`<div className="dashboard-grid" data-tour="medication-section">
        <${Card} className="span-12" title="Medicamentos del paciente" subtitle="Cada medicamento conserva su estado, cambios y notas para revisión clínica." action=${this.can('medicationsManage')?html`<${Button} icon="plus" onClick=${()=>this.openMedication(patient)}>Agregar medicamento</${Button}>`:null}>
          ${patient.medications.filter(item=>!item.archivedAt).length ? html`<div className="medication-list">${patient.medications.filter(item=>!item.archivedAt).map(medication => {
  ...
  renderModal() {
    ...
    if (modal.type === 'medicationDelete') return this.renderMedicationDeleteModal();
...
${clinicalFields ? html`<${FormField} label="Nota de actualización"><textarea rows="3" value=${draft.notes} onChange=${event => this.updateDraft('notes', event.target.value)} placeholder="Opcional. Se agregará al historial de notas."></textarea></${FormField}>`:null}<${FormActions} disabled=${this.state.formSaving} onCancel=${this.closeModal} submitLabel="Guardar cambios"/></form></${Modal}>`;
  }
  ...
  renderMedicationDeleteModal() {
    const {patientId,medicationId,draft}=this.state.modal;const patient=this.state.data.patients.find(item=>item.id===patientId);const medication=(patient?.medications||[]).find(item=>item.id===medicationId);
    return html`<${Modal} title="Eliminar medicamento" subtitle=${`${medication?.name||'Medicamento'} · ${patient?.name||''}`} onClose=${this.closeModal} size="md"><form className="clinical-form" onSubmit=${this.saveMedicationDelete}>${this.renderModalError()}<div className="form-warning"><${Icon} name="alert" size=${18}/><span>Se quitará de la lista del paciente. Linkare conservará una copia archivada con fecha, usuario y motivo para mantener la trazabilidad clínica.</span></div><${FormField} label="Motivo" required=${true}><textarea autoFocus rows="4" minLength="3" value=${draft.reason} onChange=${event=>this.updateDraft('reason',event.target.value)} placeholder="Ej. se agregó al paciente equivocado" required></textarea></${FormField}><${FormActions} disabled=${this.state.formSaving} onCancel=${this.closeModal} submitLabel=${this.state.formSaving?'Eliminando…':'Eliminar medicamento'}/></form></${Modal}>`;
  }
